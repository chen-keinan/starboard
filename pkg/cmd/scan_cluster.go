...
func NewCheckCommand(buildInfo starboard.BuildInfo, cf *genericclioptions.ConfigFlags, out io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			scheme1 := starboard.NewScheme()
			kubeClient, err := client.New(kubeConfig, client.Options{Scheme: scheme1})
			opts, err := getScannerOpts(cmd)
			if err != nil {
				return err
			}
			res, err := kubeClientset.AppsV1().RES("").List(ctx, metav1.ListOptions{})
			if err != nil {
				return err
			}
			fmt.Println(res)
